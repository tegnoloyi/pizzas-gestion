...
<div class="max-w-3xl mx-auto bg-white rounded-xl shadow-sm border border-gray-100 p-8">
    ...
    <h2 class="text-2xl font-black text-gray-800 tracking-tight mb-8">Agregar Alitas</h2>

    <form action="{{ route('alitas.store') }}" method="POST" class="space-y-6">
        @csrf

        <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Orden <span class="text-red-500">*</span></label>
            <input type="text" name="orden" required placeholder="Ej: 10 Alitas BBQ" 
        ...
        <div>
            ...
            <input type="number" step="0.01" name="precio" required placeholder="Ej: 85.00" 
                   class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-amber-400 focus:border-amber-400 outline-none text-gray-600 bg-white transition-all">
        </div>
