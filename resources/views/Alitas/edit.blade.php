...
<div class="max-w-3xl mx-auto bg-white rounded-xl shadow-sm border border-gray-100 p-8">
    ...
    <a href="{{ route('alitas.index') }}" class="inline-flex items-center gap-2 text-gray-500 hover:text-gray-800 font-medium mb-6 transition-colors">
    ...
    <h2 class="text-2xl font-black text-gray-800 tracking-tight mb-8">Editar Alitas</h2>

    <form action="{{ route('alitas.update', $alita->id_alis) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT') <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Orden <span class="text-red-500">*</span></label>
            <input type="text" name="orden" value="{{ $alita->orden }}" required 
        ...
        <div>
            ...
            <input type="number" step="0.01" name="precio" value="{{ $alita->precio }}" required 
                   class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-amber-400 focus:border-amber-400 outline-none text-gray-600 bg-white transition-all">
        </div>
        ...
        <div>
            ...
            <select name="id_cat" required class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-amber-400 focus:border-amber-400 outline-none text-gray-600 bg-white transition-all">
                ...
                @foreach($categorias as $cat)
                    <option value="{{ $cat->id_cat }}" {{ $alita->id_cat == $cat->id_cat ? 'selected' : '' }}>
                        {{ $cat->descripcion }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="flex items-center gap-3 pt-4">
            ...
            <a href="{{ route('alitas.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white px-6 py-2.5 rounded-lg text-sm font-bold transition-colors shadow-sm">
                Cancelar
            </a>
        </div>
    </form>
