...
<div x-data="{ mostrarModalEliminar: false, formAccion: '' }" class="max-w-7xl mx-auto bg-white rounded-xl shadow-sm border border-gray-100 p-8 relative">
    ...
    <div class="flex justify-between items-center mb-8 pb-4 border-b border-gray-50">
        <div>
            <h2 class="text-2xl font-black text-gray-800 tracking-tight">Alitas</h2>
            <p class="text-sm text-gray-500 mt-1">Gestiona las opciones de alitas y sus precios</p>
        </div>
        
        <a href="{{ route('alitas.create') }}" class="bg-amber-500 hover:bg-amber-600 text-white px-5 py-2.5 rounded-lg text-sm font-bold flex items-center gap-2 transition-colors shadow-sm">
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-gray-50 text-sm">
                @if($alitas->isEmpty())
                    <tr>
                        <td colspan="4" class="px-6 py-8 text-center text-gray-400 italic">No hay alitas registradas por el momento.</td>
                    </tr>
                @endif

                @foreach($alitas as $alita)
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-6 py-4 text-gray-900 font-bold text-sm">{{ $alita->orden }}</td>
                        <td class="px-6 py-4 text-green-500 font-black text-sm">${{ number_format($alita->precio, 2) }}</td>
                        <td class="px-6 py-4 text-gray-400 italic text-sm">{{ $alita->categoria }}</td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end gap-3">
                                <a href="{{ route('alitas.edit', $alita->id_alis) }}" class="text-blue-500 hover:text-blue-700 transition-colors" title="Editar">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                    </svg>
                                </a>
                                <button type="button" @click="formAccion = '{{ route('alitas.destroy', $alita->id_alis) }}'; mostrarModalEliminar = true" class="text-red-500 hover:text-red-700 transition-colors" title="Eliminar">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
    ...
    <div x-show="mostrarModalEliminar" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" x-transition.opacity>
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 overflow-hidden text-center" @click.away="mostrarModalEliminar = false" x-transition.scale.origin.bottom>
            ...
            <h3 class="text-xl font-black text-gray-900 mb-2 tracking-tight">¿Eliminar Alitas?</h3>
