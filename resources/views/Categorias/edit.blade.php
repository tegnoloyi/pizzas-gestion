...
<div class="max-w-3xl mx-auto bg-white rounded-xl shadow-sm border border-gray-100 p-8">
    ...
    <h2 class="text-2xl font-black text-gray-800 tracking-tight mb-8">Editar Categoría</h2>

    <form action="{{ route('categorias.update', $categoria->id_cat) }}" method="POST" class="space-y-6">
        @csrf @method('PUT')
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Descripción / Nombre <span class="text-red-500">*</span></label>
            <input type="text" name="descripcion" value="{{ $categoria->descripcion }}" required class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-amber-400 outline-none">
        </div>
