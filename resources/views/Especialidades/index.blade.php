...
<div x-data="{ mostrarModalEliminar: false, formAccion: '' }" class="max-w-7xl mx-auto bg-white rounded-xl shadow-sm border border-gray-100 p-8 relative">
    ...
    <div class="flex justify-between items-center mb-8 pb-4 border-b border-gray-50">
        <div>
            <h2 class="text-2xl font-black text-gray-800 tracking-tight">Especialidades</h2>
            <p class="text-sm text-gray-500 mt-1">Gestiona los sabores e ingredientes de tus pizzas</p>
        </div>
        <a href="{{ route('especialidades.create') }}" class="bg-amber-500 hover:bg-amber-600 text-white px-5 py-2.5 rounded-lg text-sm font-bold flex items-center gap-2 transition-colors shadow-sm">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" /></svg> Añadir
        </a>
    </div>
    ...
    <div x-show="mostrarModalEliminar" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" x-transition.opacity>
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 overflow-hidden text-center" @click.away="mostrarModalEliminar = false" x-transition.scale.origin.bottom>
            ...
            <h3 class="text-xl font-black text-gray-900 mb-2 tracking-tight">¿Eliminar Especialidad?</h3>
            <p class="text-gray-500 text-sm mb-6 mt-2">Ten cuidado, borrar una especialidad podría afectar a las pizzas Magno o Rectangulares que la usen.</p>
