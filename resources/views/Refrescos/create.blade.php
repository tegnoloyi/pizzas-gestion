...
<div class="max-w-3xl mx-auto bg-white rounded-xl shadow-sm border border-gray-100 p-8">
    ...
    <h2 class="text-2xl font-black text-gray-800 tracking-tight mb-8">Añadir Refresco</h2>

    <form action="{{ route('refrescos.store') }}" method="POST" class="space-y-6">
        @csrf
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Nombre <span class="text-red-500">*</span></label>
            <input type="text" name="nombre" required placeholder="Ej. Coca-Cola, Sprite..." class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-amber-400 outline-none">
        </div>
        ...
        <div>
            ...
            <select name="id_tamano" required class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-amber-400 outline-none bg-white">
                ...
                @foreach($tamanos as $tam) 
                    <option value="{{ $tam->id_tamano }}">{{ $tam->tamano }} - ${{ number_format($tam->precio, 2) }}</option> 
                @endforeach
            </select>
        </div>
        ...
        <div class="flex gap-3 pt-4">
            <button type="submit" class="bg-amber-500 hover:bg-amber-600 text-white px-6 py-2.5 rounded-lg text-sm font-bold flex items-center gap-2">Guardar Refresco</button>
