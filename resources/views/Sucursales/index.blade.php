...
<div x-data="{ mostrarModalEliminar: false, formAccion: '' }" class="max-w-7xl mx-auto bg-white rounded-xl shadow-sm border border-gray-100 p-8 relative">
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-gray-50 text-sm">
                @if($sucursales->isEmpty())
                    <tr><td colspan="5" class="px-6 py-8 text-center text-gray-400 italic">No hay sucursales registradas.</td></tr>
                @endif
                @foreach($sucursales as $suc)
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-6 py-4 text-gray-500 font-medium text-sm">#{{ $suc->id_suc }}</td>
                        <td class="px-6 py-4 text-gray-900 font-bold text-sm">{{ $suc->nombre }}</td>
                        <td class="px-6 py-4 text-gray-500 text-sm truncate max-w-xs">{{ $suc->direccion }}</td>
                        <td class="px-6 py-4 text-gray-500 text-sm">{{ $suc->telefono }}</td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end gap-3">
                                <a href="{{ route('sucursales.edit', $suc->id_suc) }}" class="text-blue-500 hover:text-blue-700 transition-colors"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg></a>
                                <button type="button" @click="formAccion = '{{ route('sucursales.destroy', $suc->id_suc) }}'; mostrarModalEliminar = true" class="text-red-500 hover:text-red-700 transition-colors"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button>
                            </div>
                        </td>
                    </tr>
                @endforeach
    ...
    <div x-show="mostrarModalEliminar" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" x-transition.opacity>
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 overflow-hidden text-center" @click.away="mostrarModalEliminar = false" x-transition.scale.origin.bottom>
            ...
            <h3 class="text-xl font-black text-gray-900 mb-2 tracking-tight">¿Eliminar Sucursal?</h3>
