...
<div x-data="{ mostrarModalEliminar: false, formAccion: '' }" class="max-w-7xl mx-auto bg-white rounded-xl shadow-sm border border-gray-100 p-8 relative">
    ...
    <div class="flex justify-between items-center mb-8 pb-4 border-b border-gray-50">
        <div>
            <h2 class="text-2xl font-black text-gray-800 tracking-tight">Tamaños de Refresco</h2>
            <p class="text-sm text-gray-500 mt-1">Gestiona los tamaños disponibles para refrescos</p>
        </div>
        <a href="{{ route('tamanos-refrescos.create') }}" class="bg-amber-500 hover:bg-amber-600 text-white px-5 py-2.5 rounded-lg text-sm font-bold flex items-center gap-2 transition-colors shadow-sm">
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-gray-50 text-sm">
                @if($tamanos->isEmpty())
                    <tr><td colspan="4" class="px-6 py-8 text-center text-gray-400 italic">No hay tamaños de refresco registrados por el momento.</td></tr>
                @endif
                @foreach($tamanos as $t)
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-6 py-4 text-gray-500 font-medium text-sm">#{{ $t->id_tamano }}</td>
    ...
    <div x-show="mostrarModalEliminar" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" x-transition.opacity>
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 overflow-hidden text-center" @click.away="mostrarModalEliminar = false" x-transition.scale.origin.bottom>
            ...
            <p class="text-gray-500 text-sm mb-6 mt-2">Atención: borrar un tamaño podría afectar a los refrescos que estén vinculados a él.</p>
